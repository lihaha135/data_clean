...
def cai_2D(save_root_path, save_root_path_cai):
    file_lst = os.listdir(save_root_path)
    file_lst = sorted(file_lst, key=lambda x: int(x[0:-4]))
    for filename in tqdm(file_lst):
        img = cv2.imread(save_root_path + filename)
        img = img[100:550, 0:2600]
        save_path = save_root_path_cai + filename
        cv2.imwrite(save_path, img)
...
save_root_path = '/media/wanji/lijingyang/data_cleaning/2d_xian/'
save_root_path_cai = '/media/wanji/lijingyang/data_cleaning/2d_xian_cai/'
draw_2D(root_path_bin, root_path_csv, save_root_path)
#cai_2D(save_root_path, save_root_path_cai)